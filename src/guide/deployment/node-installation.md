...
3. Private Key
4. User Realm (if different from master)
...
By default, the FLAME Node package deploys keycloak as part of the installation. The node clients and their secrets are
all generated and configured within this included IDP, and is used from intra-node communication authorization.
...
1. Those with access to the server running the services
   ...
   containers for each of the services and access them in their browser using the forwarded ports
...
When deploying the FLAME node without a hostname, the values file must be configured to disable routing for the
services:
...
private key for the result service.

#### Accessing the Services

Once you have deployed the FLAME Node with routing disabled, three services need to be port-forwarded:
