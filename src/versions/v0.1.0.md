...
Several default credentials were removed including keycloak and minio. Users must now specify these credentials when
deploying by either providing a username/password directly in the `values.yaml` file or by using an existing secret. The
keycloak user `flameuser` is no longer included by default unless explicitly enabled.
...
VictoriaLogs is now the primary application for storing and serving logs from all node related components and services.
It is enabled by default, but access to it's included UI is disabled as it is not currently secured behind user
authentication. If ingress for the VictoriaLogs UI is enabled, it can be found at
`https://<node hostname>/logs/select/vmui`
...
To simplify referencing this subchart in the helper functions, it was given an alias of `dataStore` which should now be
used instead
